    ...
    const result = await callModel(
      {
        ...
        prompt: getPlanningPrompt(
          researchState.topic,
          researchState.clarificationText,
        ),
...
export async function search(
  ...
  try {
    ...
    const filterResults = searchResult.results
    ...
    activityTracker.add(
      "search",
      "complete",
      `sound ${filterResults.length} result for ${query}`,
    );
...
export async function processSearchResults(
  ...
  const newFindings = extractionResults
    .filter(
      (result): result is PromiseFulfilledResult<ExtractedFindings> =>
...
export async function generateReport(
  ...
  try {
    ...
    const contextText = combineFindings(researchState.findings);
